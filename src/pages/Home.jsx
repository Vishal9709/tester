  ...
  return (
    <div className='bg-slate-200 w-full min-h-[100vh]'>
        ...
        <div className='flex flex-wrap justify-center items-center gap-6'>
          {Categories.map((item)=>{
            return <div className='w-[140px] h-[150px] bg-white flex flex-col items-start gap-5 p-5 justify-start text-[20px] font-semibold text-gray-600 rounded-lg shadow-xl hover:bg-orange-200 cursor-pointer transition-all duration-200' onClick={()=>filter(item.name)}>
              {item.icon}
              {item.name}
            </div>
          })}
        ...
        <div className={`w-full md:w-[40vw] h-[100%] fixed top-0 right-0 bg-white shadow-xl p-6 transition-all duration-500 flex flex-col items-center overflow-auto ${showCart?"translate-x-0":"translate-x-full"} `}>
          <header className='w-[100%] flex justify-between items-center'>
            ...
            <RxCross2 className='w-[30px] h-[30px] text-orange-400 text-[18px] font-semibold cursor-pointer hover:text-gray-500' onClick={()=>setShowCart(false)} />
          </header>
          ...
          <button className='w-[80%] p-3 rounded-lg bg-orange-300 text-white hover:bg-orange-400 transition-all'>Place Order</button>
          </>
          :
